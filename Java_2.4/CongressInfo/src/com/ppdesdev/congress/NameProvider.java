		...
		case ITEMS:
			for (int i = 0; i < leadersArray.length(); i++) {
				try {
					...
					result.addRow(new Object[] { i + 1, leadersArray.getJSONObject(i).getString(ServiceUpdate.JSON_FIRST), leadersArray.getJSONObject(i).getString(ServiceUpdate.JSON_LAST)});
